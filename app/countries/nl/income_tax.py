...
Note: for employees below AOW age, the combined wage-tax-plus-national-
insurance rate is already folded into the Box 1 bracket percentages, so
...
def box1_tax(annual_taxable_income: float) -> float:
...
def algemene_heffingskorting(annual_income: float) -> float:
    ...
    if annual_income >= ALGEMENE_HEFFINGSKORTING_PHASEOUT_END:
        return 0.0
    reduction = (
        annual_income - ALGEMENE_HEFFINGSKORTING_PHASEOUT_START
    ) * ALGEMENE_HEFFINGSKORTING_PHASEOUT_RATE
    return round(max(0.0, ALGEMENE_HEFFINGSKORTING_MAX - reduction), 2)
...
def annual_wage_tax_withheld(annual_gross_income: float) -> dict[str, float]:
    """
    ...
    """
    gross_tax = box1_tax(annual_gross_income)
    ahk = algemene_heffingskorting(annual_gross_income)
